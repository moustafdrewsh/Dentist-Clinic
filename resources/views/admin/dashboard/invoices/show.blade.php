@extends('layouts.master')

@section('title', 'تفاصيل الفاتورة')

@section('css')
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
@endsection

@section('page-header')
    <div class="page-header d-flex align-items-center justify-content-between border-bottom mb-4">
        <h1 class="page-title">تفاصيل الفاتورة</h1>
        <div>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">الرئيسية</a></li>
                <li class="breadcrumb-item"><a href="{{ route('invoices.index') }}">إدارة الفواتير</a></li>
                <li class="breadcrumb-item active" aria-current="page">تفاصيل الفاتورة</li>
            </ol>
        </div>
    </div>
@endsection

@section('content')
<div class="row">
    <div class="col-lg-12">
        <div class="card custom-card">
            <div class="card-header">
                <h3 class="card-title">تفاصيل الفاتورة</h3>
            </div>
            <div class="card-body">
                <p><strong>المريض:</strong> {{ $invoice->patient->user->name }}</p>
                <p><strong>المبلغ الكلي:</strong> {{ $invoice->formatCurrency($invoice->total_amount, 'SYP') }}</p>
                <p><strong>حالة الدفع:</strong> {{ ucfirst($invoice->payment_status) }}</p>
                <p><strong>تاريخ الإصدار:</strong> {{ \Carbon\Carbon::parse($invoice->issued_date)->format('d-m-Y') }}</p>
                <p><strong>تاريخ الدفع:</strong> 
                    {{ $invoice->payment_date ? \Carbon\Carbon::parse($invoice->payment_date)->format('d-m-Y') : 'لم يتم الدفع بعد' }}
                </p>

                <div class="d-flex justify-content-between">
                    <a href="{{ route('invoices.index') }}" class="btn btn-secondary">رجوع</a>
                    <a href="{{ route('invoices.edit', $invoice->id) }}" class="btn btn-warning">تعديل</a>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection

@section('js')
@endsection
